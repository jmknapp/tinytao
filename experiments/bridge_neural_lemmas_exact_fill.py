"""15.5: rounded lemma heads, then exact affine leftover. Oracle JSON last."""
...
LR = 5e-2
SEED = 20260909
PREREG = Path("tiny_tao_results/bridge_preregistration_neural_lemmas_exact_fill.json")
...
def discover_one(map_name: str, run: Path) -> dict:
    ...
    for L in chosen.lemmas:
        if L.kind == "odd_part":
            print(f"  r={L.r:>3}  odd_part({L.a}n+{L.b})  source={L.source}")
        else:
            print(f"  r={L.r:>3}  k={L.k}  F^k={L.A}q+{L.B}  source={L.source}")
...
def _r1(payload: dict) -> dict | None:
    return next((L for L in payload["lemmas"] if L["r"] == 1), None)


def _r9(payload: dict) -> dict | None:
    return next((L for L in payload["lemmas"] if L["r"] == 9), None)
...
def main() -> None:
    ...
    run = Path("tiny_tao_results") / f"bridge_neural_lemmas_exact_fill_{stamp}"
    ...
    med_r1 = _r1(medium)
    hard_r9 = _r9(hard)
    p1 = thicker["level5_gate"] and thicker["chosen_M"] == 32
    p2 = (
        medium["level5_gate"]
        and medium["chosen_M"] == 16
        and med_r1 is not None
        and med_r1["kind"] == "affine"
        and med_r1["k"] == 2
        and med_r1["A"] == 10
        and med_r1["B"] == 1
        and med_r1["source"] == "exact_affine"
    )
    p3 = not control["level5_gate"]
    p4 = (
        hard["level5_gate"]
        and hard["chosen_M"] == 16
        and hard_r9 is not None
        and hard_r9["kind"] == "affine"
        and hard_r9["A"] == 12
        and hard_r9["B"] == 7
    )
    score = {
        "P1_thicker_still_level5": p1,
        "P2_medium_level5_via_exact_fill": p2,
        "P3_control_still_fails": p3,
        "P4_hard_level5": p4,
        "P5_not_a_rescore": True,
        "preregistered_gate": p1 and p2 and p3,
        "medium_r1": med_r1,
        "hard_r9": hard_r9,
    }
    index = {
    ...
    print("PREREGISTERED GATE (P1 and P2 and P3):", score["preregistered_gate"])
